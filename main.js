...
document.addEventListener('yt-page-data-updated', async () => {
    ...
    for ( let i = 0; i < 20; ++i ) {
        try {
            settingsBtn = document.querySelector('ytd-watch-flexy.style-scope.ytd-page-manager.hide-skeleton .ytp-settings-button');
            settingsBtn.click();
			settingsBtn.click();
            break;
        }
        catch (err) { await sleep(100); }
    }
    ...
    for ( let i = 0; i < 20; ++i ) {
        try {
			let qualityBtn = document.querySelector('div.ytp-menuitem-content div:not(.ytp-menuitem-toggle-checkbox)');
			qualityBtn.click();
            qualityOptions = [...document.querySelectorAll('div.ytp-quality-menu div.ytp-menuitem')];
            break;
        }
        catch (err) { await sleep(100); }
    }
